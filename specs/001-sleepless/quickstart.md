...
sudo cp ai.pressw.sleepless.reset.plist /Library/LaunchDaemons/
sudo chown root:wheel /Library/LaunchDaemons/ai.pressw.sleepless.reset.plist
sudo launchctl bootstrap system /Library/LaunchDaemons/ai.pressw.sleepless.reset.plist

# 3) Login start (user agent)
cp ai.pressw.sleepless.plist ~/Library/LaunchAgents/
launchctl bootout gui/$(id -u)/ai.pressw.sleepless 2>/dev/null    # idempotent: bootout before bootstrap (upgrades)
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/ai.pressw.sleepless.plist
```
...
1. **SC-001/002 flip + restore**: enable → `SleepDisabled 1`, glyph ☀/▲; Quit → flag clears **and `launchctl print gui/$(id -u)/ai.pressw.sleepless` shows it is NOT relaunched** (clean exit 0, `KeepAlive{Crashed:true}`). `kill -9 <pid>` while on → launchd relaunches (≤~10s throttle) and the launch baseline clears the flag.
...
launchctl bootout gui/$(id -u)/ai.pressw.sleepless 2>/dev/null
rm -f ~/Library/LaunchAgents/ai.pressw.sleepless.plist
sudo launchctl bootout system/ai.pressw.sleepless.reset 2>/dev/null
sudo rm -f /Library/LaunchDaemons/ai.pressw.sleepless.reset.plist /etc/sudoers.d/sleepless
